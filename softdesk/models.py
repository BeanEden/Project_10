...
class Contributors(models.Model):
    user_contributing = models.ForeignKey(
        ...
        related_name='user_contributing')
...
class Project(models.Model):
    name = models.CharField(max_length=255)
    user_contributing = models.ManyToManyField('self',
        symmetrical=False,
        through="softdesk.Contributors"
    )
...
class Comment(models.Model):
    name = models.CharField(max_length=255)
    issue_associated = models.ForeignKey(to=Project, on_delete=models.CASCADE, related_name='comments')